...
def check_keydown_events(event, settings, ship=None):
    key = event.key
    if key == pg.K_SPACE: 
        ship.shooting = True
    elif key in movement.keys() and ship is not None: 
...
def check_keyup_events(event, ship=None):
    key = event.key
    if key == pg.K_SPACE: 
        if ship is not None: ship.shooting = False
    elif key == pg.K_ESCAPE: 
        ship.vel = Vector()   # Note: Escape key stops the ship

def check_events(settings, ship=None):
